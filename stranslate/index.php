...
<head>
        ...
        tailwind.config = {
            ...
            theme: {
                extend: {
                    ...
                    boxShadow: { 'input-focus': '0 0 0 3px rgba(109, 40, 217, 0.3)', 'btn': '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)', 'btn-hover': '0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)', 'dark-input-focus': '0 0 0 3px rgba(147, 51, 234, 0.5)', 'dark-btn': '0 4px 6px -1px rgba(255, 255, 255, 0.05), 0 2px 4px -2px rgba(255, 255, 255, 0.05)', 'dark-btn-hover': '0 10px 15px -3px rgba(255, 255, 255, 0.07), 0 4px 6px -4px rgba(255, 255, 255, 0.07)', 'modal': '0 10px 25px -5px rgba(0, 0, 0, 0.2), 0 20px 40px -10px rgba(0, 0, 0, 0.3)', 'dark-modal': '0 10px 25px -5px rgba(255, 255, 255, 0.08), 0 20px 40px -10px rgba(255, 255, 255, 0.1)', }
                },
            }
        }
    </script>
    ...
    <style type="text/tailwindcss">
        @layer base {
            ...
            .custom-checkbox:indeterminate { @apply bg-primary/50 border-primary/50 dark:bg-primary-light/50 dark:border-primary-light/50; background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 16 16'%3e%3cpath stroke='white' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M4 8h8'/%3e%3c/svg%3e"); }

            .modal-overlay { @apply fixed inset-0 bg-transparent z-40 opacity-0 pointer-events-none transition-opacity duration-300 ease-out; }
            .modal-container { @apply fixed inset-0 z-50 flex items-center justify-center p-4 opacity-0 pointer-events-none transition-opacity duration-300 ease-out; }
            .modal-content { @apply bg-white dark:bg-dm-card rounded-xl shadow-modal dark:shadow-dark-modal w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden transform scale-95 transition-transform duration-300 ease-out; }
            .modal-container.active { @apply opacity-100 pointer-events-auto; }
            .modal-container.active .modal-overlay { @apply opacity-100; }
            .modal-container.active .modal-content { @apply scale-100 opacity-100; }
            .modal-header { @apply p-4 border-b border-gray-200 dark:border-dm-border flex justify-between items-center; }
            .modal-title { @apply text-lg font-semibold text-gray-900 dark:text-dm-text-primary; }
            .modal-close-btn { @apply text-gray-400 hover:text-gray-600 dark:text-dm-text-muted dark:hover:text-dm-text-secondary transition-colors; }
            .modal-body { @apply p-4 overflow-y-auto flex-grow; }
            .modal-footer { @apply p-4 border-t border-gray-200 dark:border-dm-border flex flex-wrap justify-end gap-3; }
            .modal-dir-header { @apply flex items-center gap-x-2 py-2 px-1 sticky top-0 bg-gray-100 dark:bg-dm-card z-10 rounded-md; }
            .modal-dir-name { @apply font-semibold text-sm text-gray-700 dark:text-dm-text-secondary; }
            .modal-list-item { @apply grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 items-center pl-8 pr-2.5 py-2 rounded-md bg-gray-50 dark:bg-dm-input-bg hover:bg-gray-100 dark:hover:bg-opacity-75 cursor-pointer transition-colors duration-150; }
            .modal-filename-text { @apply block text-sm text-gray-800 dark:text-dm-text-primary overflow-hidden whitespace-nowrap text-ellipsis; }
            .modal-list-container::-webkit-scrollbar { width: 6px; } .modal-list-container::-webkit-scrollbar-track { @apply bg-gray-200 dark:bg-dm-input-border rounded; } .modal-list-container::-webkit-scrollbar-thumb { @apply bg-gray-400 dark:bg-gray-600 rounded; } .modal-list-container::-webkit-scrollbar-thumb:hover { @apply bg-gray-500 dark:bg-gray-500; }
        }
        @layer components {
...
<body class="bg-gray-100 dark:bg-dm-bg">
            ...
            <fieldset>
             ...
             <div class="fieldset-replacement">
                ...
                <div class="mt-4 overflow-hidden">
                 ...
                 <div id="fileSelectionDisplay" class="mt-3 text-center overflow-hidden flex flex-col items-center pb-1">
                     <span id="fileNameDisplay" class="text-sm font-medium text-gray-600 dark:text-dm-text-secondary truncate px-2 inline-block align-middle w-full">No file selected</span>
                     <div class="mt-2 flex items-center justify-center">
                         <button type="button" id="showArchiveModalButton" class="hidden btn btn-secondary !px-3 !py-1.5 !text-xs align-middle">
                             <i data-feather="list" class="w-4 h-4 mr-1.5"></i>Select Files
                         </button>
                         <span id="archiveSelectionSummary" class="text-sm text-gray-500 dark:text-dm-text-muted ml-2 align-middle inline-block"></span>
                     </div>
                 </div>
            </div>
    ...
    <div id="archiveModal" class="modal-container hidden">
        <div class="modal-overlay" id="archiveModalOverlay"></div>
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="archiveModalTitle">Select Files from Archive</h3>
                 <button type="button" class="modal-close-btn" id="modalCloseButton" aria-label="Close modal"> <i data-feather="x" class="w-5 h-5"></i> </button>
            </div>
            <div class="modal-body modal-list-container space-y-1.5" id="archiveModalList"> </div>
            <div class="modal-footer">
                <div class="flex-grow flex space-x-3">
                     <button type="button" id="modalSelectAll" class="btn btn-secondary btn-sm">Select All</button>
                     <button type="button" id="modalDeselectAll" class="btn btn-secondary btn-sm">Deselect All</button>
                </div>
                <button type="button" id="modalCancelButton" class="btn btn-sm bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-dm-input-border dark:text-dm-text-secondary dark:hover:bg-dm-border">Cancel</button>
                <button type="button" id="modalConfirmButton" class="btn btn-primary btn-sm">Confirm Selection</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const STORAGE_PREFIX = 'aiSubtitleTranslator_v13_';
        ...
        function displaySelectedFile(file) {
            ...
            const displayFileName = fullFileName.length > 50 ? `${fullFileName.substring(0, 25)}...${fullFileName.substring(fullFileName.length - 20)}` : fullFileName;
            fileNameDisplay.textContent = `Selected: ${displayFileName} (${(file.size / 1024).toFixed(1)} KB)`;
            fileNameDisplay.title = fullFileName;
            ...
            showArchiveModalButton.classList.add('hidden');
            archiveSelectionSummary.textContent = '';
        }

        function displayArchiveSelection(file, count) {
            const fullFileName = file.name;
            const displayFileName = fullFileName.length > 50 ? `${fullFileName.substring(0, 25)}...${fullFileName.substring(fullFileName.length - 20)}` : fullFileName;
            fileNameDisplay.textContent = `Archive: ${displayFileName}`;
            fileNameDisplay.title = fullFileName;
            fileNameDisplay.classList.remove('text-gray-600', 'dark:text-dm-text-secondary');
            fileNameDisplay.classList.add('text-primary-dark', 'dark:text-primary-light', 'font-semibold');
            showArchiveModalButton.classList.remove('hidden');
            feather.replace({ width: '1em', height: '1em', class: 'inline-block -mt-0.5' });
            updateArchiveSelectionSummary();
        }

        function updateArchiveSelectionSummary() {
            const count = selectedArchiveIndices.length;
            archiveSelectionSummary.textContent = count > 0 ? `(${count} file${count > 1 ? 's' : ''} selected)` : '(No files selected)';
        }

        function openArchiveModal() {
            populateArchiveModalList();
            archiveModal.classList.remove('hidden');
            requestAnimationFrame(() => { archiveModal.classList.add('active'); });
        }
        ...
        function populateArchiveModalList() {
            archiveModalList.innerHTML = '';
            archiveModalTitle.textContent = `Select Files from ${currentArchiveFilename}`;
            const filesByDirectory = currentArchiveEntries.reduce((acc, entry, index) => { const dir = getDirectory(entry.name); if (!acc[dir]) { acc[dir] = []; } acc[dir].push({ ...entry, originalIndex: index }); return acc; }, {});

            Object.keys(filesByDirectory).sort().forEach(dir => {
                const files = filesByDirectory[dir]; const dirId = `dir-${dir.replace(/[^a-zA-Z0-9]/g, '-') || 'root'}`;
                const dirHeader = document.createElement('div'); dirHeader.className = 'modal-dir-header';
                const dirCheckbox = document.createElement('input'); dirCheckbox.type = 'checkbox'; dirCheckbox.id = dirId; dirCheckbox.className = 'custom-checkbox dir-checkbox'; dirCheckbox.dataset.directory = dir;
                const dirNameLabel = document.createElement('label'); dirNameLabel.htmlFor = dirId; dirNameLabel.className = 'modal-dir-name cursor-pointer'; dirNameLabel.textContent = dir === '' ? '(Root Directory)' : dir;
                dirHeader.appendChild(dirCheckbox); dirHeader.appendChild(dirNameLabel); archiveModalList.appendChild(dirHeader);

                files.forEach(entry => {
                    const item = document.createElement('div'); item.className = 'modal-list-item'; item.dataset.directory = dir;
                    const textContainer = document.createElement('div'); textContainer.className = 'min-w-0 overflow-hidden';
                    const fileNameSpan = document.createElement('span'); fileNameSpan.className = 'modal-filename-text'; fileNameSpan.textContent = getBaseName(entry.name); fileNameSpan.title = entry.name;
                    const fileTypeSpan = document.createElement('span'); fileTypeSpan.className = 'text-xs text-gray-500 dark:text-dm-text-muted ml-1 whitespace-nowrap'; fileTypeSpan.textContent = ` (.${entry.extension})`;
                    fileNameSpan.appendChild(fileTypeSpan); textContainer.appendChild(fileNameSpan);
                    const fileCheckbox = document.createElement('input'); fileCheckbox.type = 'checkbox'; fileCheckbox.id = `modalFile_${entry.originalIndex}`; fileCheckbox.value = String(entry.originalIndex); fileCheckbox.className = 'custom-checkbox file-checkbox'; fileCheckbox.checked = selectedArchiveIndices.includes(entry.originalIndex); fileCheckbox.dataset.directory = dir;
                    item.appendChild(textContainer); item.appendChild(fileCheckbox); archiveModalList.appendChild(item);
                    item.addEventListener('click', (event) => { if (event.target.tagName === 'INPUT' && event.target.type === 'checkbox') { return; } const cb = item.querySelector('input[type="checkbox"]'); if (cb) { cb.checked = !cb.checked; updateDirectoryCheckboxState(dir); } });
                    fileCheckbox.addEventListener('change', () => { updateDirectoryCheckboxState(dir); });
                });
                dirCheckbox.addEventListener('change', (e) => { const isChecked = e.target.checked; archiveModalList.querySelectorAll(`.file-checkbox[data-directory="${dir}"]`).forEach(cb => { cb.checked = isChecked; }); });
                updateDirectoryCheckboxState(dir);
            });
        }

        function updateDirectoryCheckboxState(dir) {
            const dirCheckbox = archiveModalList.querySelector(`.dir-checkbox[data-directory="${dir}"]`); if (!dirCheckbox) return;
            const fileCheckboxes = archiveModalList.querySelectorAll(`.file-checkbox[data-directory="${dir}"]`); const totalFiles = fileCheckboxes.length; let checkedCount = 0;
            fileCheckboxes.forEach(cb => { if (cb.checked) checkedCount++; });
            if (checkedCount === 0) { dirCheckbox.checked = false; dirCheckbox.indeterminate = false; }
            else if (checkedCount === totalFiles) { dirCheckbox.checked = true; dirCheckbox.indeterminate = false; }
            else { dirCheckbox.checked = false; dirCheckbox.indeterminate = true; }
        }
        ...
        async function processUploadedFile(file) {
            ...
            if (['zip', 'xz', 'gz', 'tar.gz', 'tar.xz', '7z'].includes(fileType) || ['tar.gz', 'tar.xz'].includes(secondLastExt)) {
                ...
                try {
                    ...
                    if (fileType === 'zip') { archive = await JSZip.loadAsync(file); for (const [relativePath, zipEntry] of Object.entries(archive.files)) { if (!zipEntry.dir) { const entryName = zipEntry.name; const entryExt = entryName.split('.').pop().toLowerCase(); if (['srt', 'ass'].includes(entryExt)) { entries.push({ name: entryName, getContent: async () => zipEntry.async('string'), extension: entryExt }); } } } }
                    else if (fileType === 'xz' && !fileName.endsWith('.tar.xz')) { if (!isXZWasmLoaded) { updateStatus("XZ library not loaded.", "error"); displaySelectedFile(file); return; } const decompressedBlob = await decompressXZ(file); const entryName = fileName.replace(/\.xz$/, ''); const entryExt = entryName.split('.').pop().toLowerCase(); if (['srt', 'ass'].includes(entryExt)) { entries.push({ name: entryName, getContent: async () => decompressedBlob.text(), extension: entryExt }); } else { updateStatus(`Decompressed .xz not .srt/.ass.`, 'warning'); } }
                    else { updateStatus(`Archive type .${fileType} not fully supported.`, 'warning'); displaySelectedFile(file); return; }
                    if (entries.length > 0) { currentArchiveEntries = entries; selectedArchiveIndices = entries.map((_, i) => i); displayArchiveSelection(file, entries.length); updateStatus(`Found ${entries.length} file(s). Click 'Select Files' to modify.`, 'success'); }
                    else { updateStatus(`No .srt/.ass found in '${fileName}'.`, 'warning'); displaySelectedFile(file); }
                } catch (error) { updateStatus(`Archive error: ${error.message}`, 'error'); console.error("Archive error:", error); displaySelectedFile(file); }
            } else if (['srt', 'ass', 'txt'].includes(fileType)) { displaySelectedFile(file); currentArchiveEntries = []; selectedArchiveIndices = []; }
            else { updateStatus(`Unsupported type: .${fileType}.`, 'error'); fileInput.value = ''; fileNameDisplay.textContent = 'No file selected'; fileNameDisplay.title = ''; showArchiveModalButton.classList.add('hidden'); archiveSelectionSummary.textContent = ''; }
        }
        ...
        fileInput.addEventListener('change', async () => {
            downloadSection.classList.add('hidden'); processedResults = []; currentArchiveEntries = []; selectedArchiveIndices = []; currentArchiveFilename = ''; showArchiveModalButton.classList.add('hidden'); archiveSelectionSummary.textContent = '';
            if (fileInput.files.length > 0) { await processUploadedFile(fileInput.files[0]); }
        ...
        modalSelectAll.addEventListener('click', () => { archiveModalList.querySelectorAll('input[type="checkbox"]').forEach(cb => { cb.checked = true; }); archiveModalList.querySelectorAll('.dir-checkbox').forEach(cb => { cb.indeterminate = false; }); });
        ...
        form.addEventListener('submit', async (event) => {
            ...
            if (currentArchiveEntries.length > 0) { if (selectedArchiveIndices.length === 0) { updateStatus('No files selected from archive.', 'warning'); setButtonLoading(false); await releaseWakeLock(); return; } selectedArchiveIndices.forEach(index => { if (index >= 0 && index < currentArchiveEntries.length) { filesToProcess.push(currentArchiveEntries[index]); } }); updateStatus(`Processing ${filesToProcess.length} selected file(s)...`, 'info'); }
        ...
        function setupDownloadSection() {
            ...
            if (successfulTranslations.length === 0) { downloadSection.classList.add('hidden'); return; }
            if (successfulTranslations.length > 1) { downloadAllButtonText.textContent = `Download All (${successfulTranslations.length}) as ZIP`; }
        ...
        downloadAllButton.addEventListener('click', async () => {
            ...
            if (successfulTranslations.length === 1) { const result = successfulTranslations[0]; const originalBaseName = result.originalName.includes('/') ? result.originalName.substring(result.originalName.lastIndexOf('/') + 1) : result.originalName; const baseName = originalBaseName.substring(0, originalBaseName.lastIndexOf('.')) || originalBaseName; const langCode = getLanguageCode(); const outputFilename = `${baseName}_${langCode}.${result.extension}`; downloadSingleFile(result.translatedText, outputFilename); }
            else {
                ...
                try {
                    ...
                    successfulTranslations.forEach(result => {
                        ...
                        let finalZipName = finalZipPath; let count = 1; while(zip.file(finalZipName)) { finalZipName = `${originalDir}${baseName}_${langCode}_(${count++}).${result.extension}`; } zip.file(finalZipName, BOM + result.translatedText);
                    });
